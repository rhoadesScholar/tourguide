...
function hashTableInsert(table, numBuckets, value, emptyValue, hashCode, equals) {
    var mask = numBuckets - 1 >>> 0;
    var bucket = (hashCode & mask) >>> 0;
    for (var probe = 0; true; ++probe) {
        var x = table[bucket];
        if (x === emptyValue) {
            table[bucket] = value;
            return value;
 ...
 */
export function getNextEdge(edgeIndexAndFlipped) {
    return nextEdgeTable >>> edgeIndexAndFlipped * 3 & 7;
}
export function getBaseIndex(entry) {
    return (entry >>> 2) * 3;
}
export function getEdgeIndex(entry) {
    return entry & 3;
}
/**
 ...
 */
export function vertexAIndex(edgeIndex) {
    return edgeIndex >>> 1;
}
/**
 ...
 */
export function vertexBIndex(edgeIndex) {
 ...
 */
export function vertexCIndex(edgeIndex) {
    return 2 - edgeIndex;
}
export function getEdgeMapSize(numIndices) {
    var numEdges = numIndices;
    // Choose quadratic probing hash table size to be the smallest power of 2 greater than `numEdges`.
    var edgeMapSize = Math.pow(2, Math.ceil(_Math$log(numEdges)));
...
function computeTriangleAdjacencies(triangleAdjacencies, indices, edgeMap) {
    ...
    var edgeMapSize = edgeMap.length;
    var emptyEntry = 0xFFFFFFFF;
    triangleAdjacencies.fill(emptyEntry);
    edgeMap.fill(emptyEntry);
    // Insert edges
    for (var triangle = 0; triangle < numTriangles; ++triangle) {
        var baseIndex = triangle * 3;

        var _loop = function _loop(edgeIndex) {
            var vertexA0 = indices[baseIndex + vertexAIndex(edgeIndex)];
            var vertexB0 = indices[baseIndex + vertexBIndex(edgeIndex)];
            var newEntry = triangle << 2 | edgeIndex;
            var existingEntry = hashTableInsert(edgeMap, edgeMapSize, newEntry, emptyEntry, hashEdge(vertexA0, vertexB0), function (x) {
                var otherBaseIndex = getBaseIndex(x);
                var otherEdgeIndex = getEdgeIndex(x);
                var vertexA1 = indices[otherBaseIndex + vertexAIndex(otherEdgeIndex)];
                var vertexB1 = indices[otherBaseIndex + vertexBIndex(otherEdgeIndex)];
            ...
            if (existingEntry !== newEntry) {
                var otherBaseIndex = getBaseIndex(existingEntry);
                var otherEdgeIndex = getEdgeIndex(existingEntry);
...
function emitTriangleStrips(indices, triangleAdjacencies, output, outputIndex) {
    var invalidVertex = ~0 >>> 32 - 8 * output.BYTES_PER_ELEMENT;
    var numIndices = indices.length;
    var numTriangles = numIndices / 3;
    var emptyEntry = 0xFFFFFFFF;
    // Extract strips
    startNewStrip: for (var triangle = 0; triangle < numTriangles; ++triangle) {
        var baseIndex = triangle * 3;
        ...
        for (var edgeIndex = 0; edgeIndex < 3; ++edgeIndex) {
            var entry = triangleAdjacencies[baseIndex + edgeIndex];
            if (entry === emptyEntry) continue;
            var otherBaseIndex = getBaseIndex(entry);
            if (indices[otherBaseIndex] === invalidVertex) continue;
            var otherEdgeIndex = getEdgeIndex(entry);
            ...
            var edgeIndexAndFlipped = otherEdgeIndex;
            while (true) {
...
export function computeTriangleStrips(indices, subChunkOffsets) {
    if (indices.length === 0) return indices;
    collisions = 0;
    if (subChunkOffsets === undefined) {
        subChunkOffsets = Uint32Array.of(0, indices.length);
    }
    var adjacenciesElapsed = 0;
    var emitElapsed = 0;
    var startTime = 0,
        midTime = 0,
        endTime = 0;
    var maxVertexIndex = normalizeTriangleVertexOrder(indices);
    var outputBufferSize = indices.length / 3 * 4;
    var output = maxVertexIndex >= 65535 ? new Uint32Array(outputBufferSize) : new Uint16Array(outputBufferSize);
    var outputIndex = 0;
    var maxSubChunkIndices = 0;
    var numSubChunks = subChunkOffsets.length - 1;
    for (var subChunk = 0; subChunk < numSubChunks; ++subChunk) {
        maxSubChunkIndices = Math.max(maxSubChunkIndices, subChunkOffsets[subChunk + 1] - subChunkOffsets[subChunk]);
    }
    var triangleAdjacencies = new Uint32Array(maxSubChunkIndices);
    var edgeMap = new Uint32Array(getEdgeMapSize(maxSubChunkIndices));
    var subChunkOffset = subChunkOffsets[0];
    for (var _subChunk = 0; _subChunk < numSubChunks; ++_subChunk) {
        subChunkOffsets[_subChunk] = outputIndex;
        var subChunkEnd = subChunkOffsets[_subChunk + 1];
        var subIndices = indices.subarray(subChunkOffset, subChunkEnd);
    ...
    var shrunkOutput = new output.constructor(outputIndex);
...
//# sourceMappingURL=triangle_strips.js.map