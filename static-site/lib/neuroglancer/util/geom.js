...
export var identityMat4 = mat4.create();
export var AXES_NAMES = ['x', 'y', 'z'];
export var BoundingBox = function BoundingBox(lower, upper) {
    _classCallCheck(this, BoundingBox);

    this.lower = lower;
    this.upper = upper;
};
export var kAxes = [vec3.fromValues(1, 0, 0), vec3.fromValues(0, 1, 0), vec3.fromValues(0, 0, 1)];
export var kZeroVec = vec3.fromValues(0, 0, 0);
export var kOneVec = vec3.fromValues(1, 1, 1);
export var kInfinityVec = vec3.fromValues(Infinity, Infinity, Infinity);
export var kIdentityQuat = quat.create();
export function prod3(x) {
    return x[0] * x[1] * x[2];
}
export function prod4(x) {
 ...
 */
export function vec3Key(x) {
    return x[0] + ',' + x[1] + ',' + x[2];
}
var RECTIFY_EPSILON = 1e-4;
export function rectifyVec3IfAxisAligned(v, offset) {
 ...
 */
export function rectifyTransformMatrixIfAxisAligned(m) {
 ...
 */
export function quatRotateX180(out, a) {
    var x = a[0],
        y = a[1],
        z = a[2],
        w = a[3];
 ...
 */
export function quatRotateY180(out, a) {
    var x = a[0],
        y = a[1],
        z = a[2],
        w = a[3];
 ...
 */
export function quatRotateZ180(out, a) {
    var x = a[0],
        y = a[1],
        z = a[2],
        w = a[3];
 ...
 * `m` is ignored.
 */
export function transformVectorByMat4(out, a, m) {
    var x = a[0],
        y = a[1],
        z = a[2];
 ...
 */
export function effectiveScalingFactorFromMat4(out, m) {
    var m0 = m[0],
        m1 = m[1],
        m2 = m[2],
        m4 = m[4],
...
export function translationRotationScaleZReflectionToMat4(out, translation, rotation, scale, zReflection) {
    var temp = out;
    out[0] = scale[0];
    out[1] = scale[1];
    out[2] = scale[2] * zReflection;
    return mat4.fromRotationTranslationScale(out, rotation, translation, temp);
}
/**
 * Returns the value of `t` that minimizes `(p - (a + t * (b - a)))`.
 */
export function findClosestParameterizedLinePosition(a, b, p) {
    // http://mathworld.wolfram.com/Point-LineDistance3-Dimensional.html
    // Compute t: -dot(a-p, b-a) / |b - a|^2
    var denominator = vec3.squaredDistance(a, b);
    var numerator = 0;
    for (var i = 0; i < 3; ++i) {
        var aValue = a[i];
 ...
 */
export function projectPointToLineSegment(out, a, b, p) {
    var t = findClosestParameterizedLinePosition(a, b, p);
...
export function mat3FromMat4(out, m) {
    var m00 = m[0],
        m01 = m[1],
        m02 = m[2],
        m10 = m[4],
 ...
 */
export function getFrustrumPlanes(out, m) {
    ...
    var nearA = m30 + m20; // near: a
    var nearB = m31 + m21; // near: b
    var nearC = m32 + m22; // near: c
    var nearD = m33 + m23; // near: d
    // Normalize near plane
    var nearNorm = Math.sqrt(Math.pow(nearA, 2) + Math.pow(nearB, 2) + Math.pow(nearC, 2));
 ...
 */
export function isAABBVisible(xLower, yLower, zLower, xUpper, yUpper, zUpper, clippingPlanes) {
    for (var i = 0; i < 6; ++i) {
        var a = clippingPlanes[i * 4],
            b = clippingPlanes[i * 4 + 1],
            c = clippingPlanes[i * 4 + 2],
            d = clippingPlanes[i * 4 + 3];
        var sum = Math.max(a * xLower, a * xUpper) + Math.max(b * yLower, b * yUpper) + Math.max(c * zLower, c * zUpper) + d;
        if (sum < 0) {
            return false;
        }
    }
