 ...
 */
import { getFrustrumPlanes, isAABBVisible, mat4 } from './geom';
describe('getFrustrumPlanes', function () {
    it('works for simple example', function () {
        var m = mat4.perspective(mat4.create(), Math.PI / 2, 4.0, 7, 113);
        var planes = getFrustrumPlanes(new Float32Array(24), m);
        var expectedPlanes = [
        // left
        +0.25, 0, -1, 0,
        // right
        ...
        planes.every(function (x, i) {
            return expect(x).toBeCloseTo(expectedPlanes[i]);
        });
    });
});
describe('isAABBVisible', function () {
    it('works for simple example', function () {
        var m = mat4.perspective(mat4.create(), Math.PI / 2, 4.0, 7, 113);
        var planes = getFrustrumPlanes(new Float32Array(24), m);
...
//# sourceMappingURL=geom.spec.js.map