 ...
 */
import { normalizeStringLiteral, pythonLiteralParse, pythonLiteralToJSON, urlSafeParse, urlSafeStringify, urlSafeStringifyString, urlSafeToJSON } from './json';
describe('url safe json', function () {
    it('urlSafeStringifyString', function () {
        expect(urlSafeStringifyString('a')).toBe('\'a\'');
        expect(urlSafeStringifyString('a"')).toBe('\'a"\'');
        expect(urlSafeStringifyString('a\'')).toBe('\'a\\\'\'');
    });
    it('urlSafeStringify', function () {
    ...
    it('urlSafeStringToJSONString', function () {
        expect(normalizeStringLiteral('\'abc\'')).toBe('"abc"');
        expect(normalizeStringLiteral('\'abc"\'')).toBe('"abc\\""');
        expect(normalizeStringLiteral('\'abc\\"\'')).toBe('"abc\\""');
    });
    it('urlSafeParse', function () {
    ...
    it('urlSafeRoundtrip', function () {
        function testRoundTrip(x) {
    ...
    it('pythonLiteralToJSON', function () {
        expect(pythonLiteralToJSON('{\'a\':\'b\', \'c\':True, \'d\':(1,2,3,), }')).toBe('{"a":"b", "c":true, "d":[1,2,3]}');
    });
    it('pythonLiteralParse', function () {
        expect(pythonLiteralParse('{\'a\':\'b\', \'c\':True, \'d\':(1,2,3,), }')).toEqual({ 'a': 'b', 'c': true, 'd': [1, 2, 3] });
    });
});
