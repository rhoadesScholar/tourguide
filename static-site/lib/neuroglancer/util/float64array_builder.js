 ...
 */
export var Float64ArrayBuilder = function () {
    function Float64ArrayBuilder() {
        var initialCapacity = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 16;

        _classCallCheck(this, Float64ArrayBuilder);

        this.length = 0;
        this.data = new Float64Array(initialCapacity);
    }

    _createClass(Float64ArrayBuilder, [{
        key: "resize",
        value: function resize(newLength) {
            var data = this.data;

            if (newLength > data.length) {
                var newData = new Float64Array(Math.max(newLength, data.length * 2));
...
//# sourceMappingURL=float64array_builder.js.map